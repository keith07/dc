...
public abstract class AbstractAlarmTrigger<T extends IAlarmEventTarget,D extends AlarmData,E extends AbstractEventEntity>{
	 ...
	 *	+-------------------------------------------------------------------------------+
	 *	|	性能警报		|	持续3分钟	|	持续3分钟	|	持续 5分钟且吞吐量 >= 5rpm	|
	 *	+-------------------------------------------------------------------------------+
	...
	private void tryTriggerAlarmEvent(int alarmLevel,AlarmDataKey<D> alarmDataKey){
		...
		//检查是否发送报警通知
			triggerEventNotification(parseTarget(alarmDataKey),event,alarmDataKey.getTimestamp());
		}
	}
	 ...
	 * @param timestamp
	 */
	private void triggerEventNotification(T target,T event,int timestamp){
		//如果警报类型为不警报，则跳过
	 ...
	 * @param timestamp
	 */
	private void triggerCloseEventNotification(T target,T event,int timestamp){
